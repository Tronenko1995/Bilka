                ...
                function showButtonProgress() {
                    ...
                    $promise.then( function(response) {
                        ...
                        if (response.status === "ok") {
                            ...
                            const tovarName = $('h1[itemprop="name"]').text();
                            const tovarImg = $('.page-card__container-left-photo-slider-item .page-card__container-left-photo-img img').attr('src');
                            const tovarUrl = document.location.href;
                            const tovarPrice = parseInt( $button.closest('form').find(".js-product-price").attr('data-price') );
                            ...
                            if ($('.jsSkusProducts').attr('skuscount') >= 2) { //если 2 и более
                                ...
                                const firstImgId = $(infoBlock).closest('.jsSkusProducts').find('li').first().find('meta[itemprop="imageId"]').attr('content');//айди белого
                                const productImgId = $(infoBlock).siblings('meta[itemprop="imageId"]').attr('content');//айди серого
                                ...
                                if (cartItems.find(`.cart__center-item[product-id='${tovarId}']`).length != 0 && cartItems.find(`.cart__center-item[data-id='${response.data.item_id}']`).length  != 0 ) {
                                    ...
                                    <div class="cart__center-item-left">\
                                        <a class="cart__center-item-left-img" href="${tovarUrl}" title="${tovarName}">\
                                            <img width="96" height="96" alt="${tovarName}" title="${tovarName}" src="${NewProductImgSrc}">\
                                        </a>\
                                    </div>\
