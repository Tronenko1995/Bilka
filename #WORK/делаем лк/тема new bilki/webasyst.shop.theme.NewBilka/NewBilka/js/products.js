( function($, waTheme) {
    ...
    function hideMore() {
        if ( $('.s-paging-list li:last-child').hasClass('selected')) {
            console.log('Это последняя страница');
            $('.main-container__for-more').css('display','none');
        }
    }
    ...
    var LazyLoading = ( function($) {
        ...
        LazyLoading = function(options) {
            ...
            $(document).on("click", ".jsMoreItems", function (e) {
            ...
            function showLoad() {
                ...
                */
                ...
                $paging
                    ...
                    /*.append($loading);*/
            $('.main-container__more-svg').toggleClass('active');
            }
    ...
    var Products = ( function($) {
        ...
        Products = function(options) {
            ...
            that.$products = that.$wrapper.find(".main-container__item");
        ...
        Products.prototype.bindEvents = function() {
            ...
            $('.page-category__wrap').on("submit", "form.add-to-cart", function(event) {
                console.log('тыкё1');
                event.preventDefault();
                that.onSubmitProduct( $(this) );
            });

            $('.page-category__wrap').on("click", ".s-compare-button", function(event) {
                event.preventDefault();
                that.onCompareProduct( $(this) );
            });


            $('.recommended-products').on("submit", "form.add-to-cart", function(event) {
                console.log('тыкё2');
                event.preventDefault();
                that.onSubmitProduct( $(this) );
            });

            $('.recommended-products').on("click", ".s-compare-button", function(event) {
                event.preventDefault();
                that.onCompareProduct( $(this) );
            });
        ...
        Products.prototype.initLazy = function() {
            ...
            new LazyLoading({
                $wrapper: that.$wrapper,
                names: {
                    list: ".main-container__items",
                    items: ".main-container__items",
        ...
        Products.prototype.onSubmitProduct = function($form) {
            var that = this,
                $product = $form.closest(".main-container__item"),
            ...
            if (product_href) {
                ...
                $.post($form.attr('action') + '?html=1', $form.serialize(), function(response) {
                    if (response.status === "ok") {
                        console.log('%c NewBilka -> Вы купили товар со страницы с товарами', 'color: green');
                        const tovarWrap = $button.closest('.main-container__item');
                        const tovarId = $(tovarWrap).attr('data-product-id');
                        const tovarName = $(tovarWrap).find('.main-container__item-full-link').attr('title');
                        const tovarImg = $(tovarWrap).find('img[itemprop="image"]').attr('src');
                        const tovarUrl = $(tovarWrap).find('.main-container__item-full-link').attr('href');
                        const tovarPrice = parseInt( $(tovarWrap).find(".main-container__item-for-price meta[itemprop='price']").attr('content') );
                        console.log(`Продукт ${tovarId} добавлен в корзину ${response.data.item_id} `);
                        // console.log(response);
                        const cartItems = $('.cart__center-items');
                        if (cartItems.find(`.cart__center-item[product-id='${tovarId}']`).length != 0) {
                            
                            console.log('нашел товар в корзине юзера!');
                            const thisLineTovar = cartItems.find(`.cart__center-item[product-id='${tovarId}']`);
                            ...
                            console.log('товара в корзине юзера не нашел!');
                            ...
                            <div class="cart__center-item-left">\
                                <a class="cart__center-item-left-img" href="${tovarUrl}" title="${tovarName}">\
                                    <img width="96" height="96" alt="${tovarName}" title="${tovarName}" src="${tovarImg}">\
                                </a>\
                            </div>\
        ...
        Products.prototype.onCompareProduct = function( $button ) {
            var that = this,
                ...
                $product = $button.closest(".main-container__item"),
